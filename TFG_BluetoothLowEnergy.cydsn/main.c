...
static uint16 temperatura=99;     //Temperatura total
static uint8 temperatura1=0;     //Primer Byte de temperatura
static uint8 temperatura2=0;     //Segundo Byte de temperatura
static int temperaturaant=200; //Temperatura inicial para la comparación

static uint16 humedad=99;        //Humedad total
static uint8 humedad1=0;        //Primer Byte de humedad
static uint8 humedad2=0;        //Segundo Byte de humedad
...
int lectura_DHT22(){                        //Obtener los datos de temperatura y humedad del sensor DHT22  
   ...
   //Se asignan los bytes obtenidos a los bytes que pertenecen a humedad y a temperatura.
    ...
    humedad     = (humedad1 << 8) + humedad2;          //Cálculo humedad final.
    temperatura  = (temperatura1 << 8) + temperatura2; //Cálculo temperatura final.
    ...
    { 
        ...
        if(deviceConnected & startNotificationHumidity){  // Si el dispositivo se conecta y se notifica en la característica de humedad.    
            ...
            if (humedad!=humedadant){                     // Si la humedad leida no es igual al dato almacenado en la variable "humedadant". 
                ...
                CyBle_GattsWriteAttributeValue(&notificationHandle,0,&connectionHandle, CYBLE_GATT_DB_LOCALLY_INITIATED);
            ...
            if (temperatura!=temperaturaant){               // Si la temperatura leida no es igual al dato almacenado en la variable "temperaturaant".
                ...
                setCursor(6,1);                             //Primera linea LCD. 
                ...
                CyBle_GattsWriteAttributeValue(&notificationHandle1,0,&connectionHandle, CYBLE_GATT_DB_LOCALLY_INITIATED);
            ...
            if (proximidad!=proximidadant){                 // Si el dispositivo se conecta y se notifica en la característica de proximidad.    
                ...
                CyBle_GattsWriteAttributeValue(&notificationHandle2,0,&connectionHandle, CYBLE_GATT_DB_LOCALLY_INITIATED);
